...
var websocket_stream_port_1 = 8083;
    ...
    req.on
      (
        ...
        function (data) {
          ...
          streaming_websocket_1.broadcast(data);
...
var streaming_websocket_1 = new websocket.Server({ port: websocket_stream_port_1, perMessageDeflate: false });

streaming_websocket.connectionCount = 0;
streaming_websocket_1.connectionCount = 0;




streaming_websocket_1.on
  (
    "connection",
    function (socket, upgradeReq) {
      streaming_websocket_1.connectionCount++;
      console.log
        (
          'New websocket Connection: ',
          (upgradeReq || socket.upgradeReq).socket.remoteAddress,
          (upgradeReq || socket.upgradeReq).headers['user-agent'],
          '(' + streaming_websocket_1.connectionCount + " total)"
        );
      ...
      socket.on
        (
          ...
          function (code, message) {
            streaming_websocket_1.connectionCount--;
            console.log('Disconnected websocket (' + streaming_websocket_1.connectionCount + ' total)');
          }
        );
    }
  );
...
streaming_websocket_1.broadcast = function (data) {
  streaming_websocket_1.clients.forEach
    (
      function each(client) {
        if (client.readyState === websocket.OPEN) {
          client.send(data);
